...
if init_from == 'scratch':
    ...
    for k in ['n_layer', 'n_head', 'n_embd', 'block_size', 'bias', 'vocab_size', 'lora_rank', 'lora_alpha']:
        try:
            ...
            model_args[k] = 0 # 99% chance this is because the checkpoint didn't include lora
    # create the model
    ...
    layer_count = 0
    num_of_layer = len(list(model.children()))
    if(freeze):
        for child in model.children():
            if(layer_count == (num_of_layer - 1)):
                if(freeze):
            ...
            for param in child.parameters():
            #print(param)
                param.requires_grad = freeze #not freeze
    ...
    if (iter_num % eval_interval == 0 and master_process) or (iter_num == checkpoint_iter_num and master_process):
        ...
        if (losses['val'] < best_val_loss) or (always_save_checkpoint) or (iter_num==checkpoint_iter_num):
            ...
            if iter_num > 0:
                ...
                torch.save(checkpoint, os.path.join(out_dir, outfilename))
